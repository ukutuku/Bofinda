...
async function kraevIsoleretForbindelse() {
  ...
  if (d !== 'bofinda_test' || p !== 55432) {
    console.error(`FEJL: forbundet til ${d} paa ${a}:${p} — ikke den isolerede testbase.`)
    console.error('      Proeven SKRIVER og koerer ikke mod andet.')
    await luk()
    process.exit(1)
  }
  console.log(`  forbindelse efterproevet: ${d} paa ${a}:${p}`)
}
